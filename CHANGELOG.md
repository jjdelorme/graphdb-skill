...
### Fixed
- **Ingest:** Refactored the directory walker to dynamically ignore nested git repositories and worktrees by checking for a `.git` entry, and rely completely on the project's `.gitignore` rules for skipping dependency/build artifacts (like `node_modules` and virtual environments).
...
## [1.8.414-beta] - 2026-06-04 [Pre-release]
### Fixed
- **Query:** Converted `time.Now()` to `time.Now().UTC()` in `CreateBatchJobNode` and `UpdateBatchJobNodeStatus` to avoid Neo4j timezone serialization errors (`Illegal zone identifier: "Local"`).
...
## [1.6.373-beta] - 2026-05-13 [Pre-release]
### Fixed
- **Enrich:** Fixed a bug where `-dir` was ignored during the extraction phase of `enrich-features`, causing file-not-found errors when run from outside the project root.
- **Query:** Added `-dir` flag support to `fetch-source` and `locate-usage` query types to ensure correct source file resolution.
